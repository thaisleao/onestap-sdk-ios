...
class ApiUserTests: XCTestCase {
    ...
    func testFailedApiAccountJsonToObject() {
        let data = self.accountUnsuccessfulJSON.data(using: String.Encoding.utf8)
        let apiResponse = try? ApiResponse<ApiAccount>(data: data, httpUrlResponse: HTTPURLResponse(statusCode: 500))
        XCTAssertFalse(apiResponse?.success ?? true)
    }
