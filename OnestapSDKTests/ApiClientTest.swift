...
class ApiClientTest: XCTestCase {
    ...
    func test_execute_successful_http_response_parses_ok() {
        ...
        let expectedUtf8StringResponse = "{ \"SomeProperty\" : \"SomeValue\", \"success\": true }"
    ...
    func test_execute_non_2xx_response_code() {
        let expectedUtf8StringResponse = "{ \"SomeProperty\" : \"SomeValue\", \"success\": false, \"operationReport\": [{ \"field\": \"Token\", \"message\": \"Token not found\" }] }"
        let expectedData = expectedUtf8StringResponse.data(using: .utf8)
        ...
        apiClient.execute(request: TestDoubleRequest()) { (result: Result<ApiResponse<TestDoubleApiEntity>>) in
            ...
            do {
                ...
                XCTAssertTrue(error.operationReport.first?.field == "Token", "The field response is not the expected one")
            } catch {
                XCTFail("Expected api error to be thrown")
            }
